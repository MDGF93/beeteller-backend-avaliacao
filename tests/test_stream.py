...
def test_start_stream_no_messages(client: TestClient, db_session):
    ...
    # Make a request to start a stream
...
def test_start_stream_with_message(client: TestClient, db_session, test_message):
    ...
    message = (
        db_session.query(PixMessage).filter(PixMessage.id == test_message["id"]).first()
    )
...
def test_start_stream_multiple_messages(client: TestClient, db_session, test_message):
    ...
    response = client.get(
        "/api/pix/12345678/stream/start", headers={"Accept": "multipart/json"}
    )
...
def test_continue_stream(client: TestClient, db_session, test_stream, test_message):
    ...
    # Make a request to continue the stream
    ...
    assert "Pull-Next" in response.headers
    assert (
        f"/api/pix/12345678/stream/{test_stream['stream_id']}"
        in response.headers["Pull-Next"]
    )
...
def test_continue_stream_invalid_id(client: TestClient, db_session):
    ...
    # Make a request with a non-existent stream ID
...
def test_terminate_stream(client: TestClient, db_session, test_stream, test_message):
    ...
    # Make a request to terminate the stream
    ...
    stream = (
        db_session.query(MessageStream)
        .filter(MessageStream.stream_id == test_stream["stream_id"])
        .first()
    )
...
def test_parallel_streams(client: TestClient, db_session):
    ...
    streams_to_deactivate = (
        db_session.query(MessageStream)
        .filter(MessageStream.ispb == "12345678", MessageStream.is_active == True)
        .all()
    )
